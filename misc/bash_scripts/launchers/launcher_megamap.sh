...
MEMORY=300g # "$SLURM_MEM_PER_NODE" # Might need to add "g" suffix.
...
mkdir /REDACTED_HPC_PATH/ynp/megamap/
output_dir=/REDACTED_HPC_PATH/ynp/megamap/
qtrim12=/REDACTED_HPC_PATH/ynp/raw_reads/all_qtrimmed.fq.gz
merged=/REDACTED_HPC_PATH/ynp/raw_reads/all_merged.fq.gz
logfile=/REDACTED_HPC_PATH/ynp/megamap/megamap_log.txt

RVMT_contigs=/REDACTED_HPC_PATH/rolypoly/data/RVMT/dust_maked_w10_tmp_nochimeras.fasta
NCBI_ribovirus=/REDACTED_HPC_PATH/rolypoly/data/NCBI_ribovirus/refseq_ribovirus_genomes.fasta
Partiti22=/REDACTED_HPC_PATH/ynp/meta/partiti_cell22.fasta
Japan_hotsprings_sequences=/REDACTED_HPC_PATH/rolypoly/data/NCBI_ribovirus/Japan_hotsprings_sequences.fasta
all_ref=$(echo $Partiti22 $NCBI_ribovirus $RVMT_contigs $Japan_hotsprings_sequences)
cat $(echo $Partiti22 $NCBI_ribovirus $RVMT_contigs $Japan_hotsprings_sequences) | seqkit seq -m 200 -w0 > tmp_all_refrences.fasta

cd $output_dir

mkdir raw_reads_mmdb searchmmdb_rvmt searchmmdb_NCBI_ribovirus tmp all_ref_mm searchmmdb_refs_r all_ynp_contigs search_all_ynp_contigs
# mmseqs createdb $qtrim12 $merged raw_reads_mmdb/rdb   #--threads $THREADS
rawreaddb="$output_dir/raw_reads_mmdb/rdb"
all_spids_contigs=/REDACTED_HPC_PATH/ynp/raw_reads/spids/cross_assembly/merged_contigs_clst_rep_seq.fasta
combined_assembly_contigs=/REDACTED_HPC_PATH/ynp/combined_assembly/merged_contigs.fasta
mmseqs easy-linclust $combined_assembly_contigs $all_spids_contigs "$output_dir"/all_merged_linclust_covmode5_c99_id_99 "$output_dir"/tmp/ --min-seq-id 0.99 -c 0.99 --threads $THREADS   --cov-mode 1
mmseqs createdb all_merged_linclust_covmode5_c99_id_99_rep_seq.fasta all_ynp_contigs/rdb   #--threads $THREADS
contigdb="$output_dir"/all_ynp_contigs/rdb

## Map all using bbwarp
# bbwrap.sh in=$qtrim12,$merged ref=tmp_all_refrences.fasta out=bbwrap_mid75_RVMT_NCBIRibo_Cell22_Serra.sam threads=$THREADS  nodisk  outm=bbwrap_mid75_RVMT_NCBIRibo_Cell22_Serra_qtrimmed.fq.gz,bbwrap_mid75_RVMT_NCBIRibo_Cell22_Serra_merged.fq.gz maxindel=200 minid=90 untrim ambig=best ow=t append=t
# bbwrap.sh in=$qtrim12,$merged ref=tmp_all_refrences.fasta out=/dev/null threads=$THREADS  nodisk outm=bbwrap_mid75_RVMT_NCBIRibo_Cell22_Serra_qtrimmed.fq.gz,bbwrap_mid75_RVMT_NCBIRibo_Cell22_Serra_merged.fq.gz maxindel=200 minid=75 untrim ambig=best ow=t append=t

## Map all using strobealign
# strobealign tmp_all_refrences.fasta $qtrim12  -o strobealign_all_refrences_qtrimmed.sam  --interleaved  -x --threads=$THREADS
# strobealign tmp_all_refrences.fasta $merged  -o strobealign_all_refrences_merged.sam  --interleaved  -x --threads=$THREADS


# ## Search for reads via MMseqs2 which allows multiple HSPs on different target seqs (easily)
# # ## map to RVMT + NCBI Ribovirus + seqs of interest
    logit $logfile "Started mapping to RVMT, reads :  $qtrim12 and $merged"
    # mmseqs createdb  tmp_all_refrences.fasta all_ref_mm/refdb
    refdb="$output_dir"/all_ref_mm/refdb
    # contigs4eval=/REDACTED_HPC_PATH/rolypoly/data/RVMT/dust_maked_w10_tmp_nochimeras.fasta
    # mmseqs createdb  $contigs4eval /REDACTED_HPC_PATH/rolypoly/data/RVMT/mmdb/RVMT_mmseqs_db2
    # rvmtdb=/REDACTED_HPC_PATH/rolypoly/data/RVMT/mmdb/RVMT_mmseqs_db2
    resdb="$output_dir"/search_all_ynp_contigs/res
    mmseqs search $contigdb  $refdb  $resdb ./tmp/  --min-seq-id 0.75 --min-aln-len 75 -e 0.0001 --search-type 3 --threads $THREADS -a # --format-mode 1   --force-reuse
    mmseqs convertalis $contigdb  $refdb  $resdb All_ynp_contigs_vs_refs_mm.tab --format-mode 4 --format-output qheader,theader,qlen,tlen,qstart,qend,tstart,tend,alnlen,mismatch,qcov,tcov,bits,evalue,gapopen,pident,nident
    # mmseqs convertalis $contigdb  $refdb  $resdb All_ynp_contigs_vs_refs_mm.sam --format-mode 1  --search-type 3
        mmseqs convertalis $contigdb  $refdb  $resdb All_ynp_contigs_vs_refs_mm.tab --format-mode 4 --format-output qheader,theader,qlen,tlen,qstart,qend,tstart,tend,alnlen,mismatch,qcov,tcov,bits,evalue,gapopen,pident,nident
        # mmseqs convertalis $contigdb  $refdb  $resdb All_ynp_contigs_vs_refs_mm_hitseqonly.tab --format-mode 4 --format-output qheader,qseq --search-type 3
...
#     # mmseqs createdb  $contigs4eval /REDACTED_HPC_PATH/rolypoly/data/RVMT/mmdb/RVMT_mmseqs_db2
#     # rvmtdb=/REDACTED_HPC_PATH/rolypoly/data/RVMT/mmdb/RVMT_mmseqs_db2
#     resdb="$output_dir"/searchmmdb_refs_r/res
#     mmseqs search $rawreaddb  $refdb  $resdb ./tmp/  --force-reuse --min-seq-id 0.75 --min-aln-len 75 -e 0.0001 --search-type 3 --threads $THREADS -a # --format-mode 1  
#     mmseqs convertalis $rawreaddb  $refdb  $resdb All_ynp_vs_RVMT_mm.tab --format-mode 4 --format-output qheader,theader,qlen,tlen,qstart,qend,tstart,tend,alnlen,mismatch,qcov,tcov,bits,evalue,gapopen,pident,nident
#     mmseqs convertalis $rawreaddb  $refdb  $resdb All_ynp_vs_RVMT_mm.sam --format-mode 1  --search-type 3
...
# ## map to NCBI RiboVirus
#     logit $logfile "Started mapping to NCBI RiboVirus, reads :  $qtrim12 and $merged"
...
# logit $logfile "Started assembly evaluation on:  $(echo $contigs4eval)"
# statswrapper.sh $(echo $contigs4eval) format=3 out="$output_dir"/coassemblies_stats.tsv
