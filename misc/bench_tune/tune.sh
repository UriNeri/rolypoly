...
awk -v RS='\t' '/ITS SP ID/{print NR; exit}' /REDACTED_HPC_PATH/rolypoly/rolypoly/metaTs_for_rolypoly_tunning.tsv
awk -F'\t' '{print $23}' /REDACTED_HPC_PATH/rolypoly/rolypoly/metaTs_for_rolypoly_tunning.tsv | tail |sort -u -r   > spids.tsv



cd /REDACTED_HPC_PATH/rolypoly/bench/
...
cd /REDACTED_HPC_PATH/rolypoly/bench/metaTs

jamo  fetch assembled.fna -s dori spid  $(cat spids.tsv)  > jamo_info.tsv
awk -F' ' '$2=="RESTORED" {print $1}' ./jamo_info.tsv  | xargs -I %  cp % ./

cp /REDACTED_HPC_PATH/scratch/dsi/aa/dm_archive/img/submissions/65383/65383.assembled.fna ./
cp /REDACTED_HPC_PATH/scratch/dsi/aa/dm_archive/img/submissions/61747/61747.assembled.fna ./
cp /REDACTED_HPC_PATH/scratch/dsi/aa/dm_archive/img/submissions/183094/183094.assembled.fna ./
cp /REDACTED_HPC_PATH/scratch/dsi/aa/dm_archive/img/submissions/76857/76857.assembled.fna ./
...
# cat /REDACTED_HPC_PATH/rolypoly/data/RVMT/RiboV1.6_Contigs.fasta combined_metaTs.fas > metaTs_spiced_RVMT.fasta
...
known_DNA_present=/REDACTED_HPC_PATH/rolypoly/bench/GCF_000025685.1_ASM2568v1_genomic.fna

cd /REDACTED_HPC_PATH/rolypoly/bench

fastq_file=/REDACTED_HPC_PATH/rolypoly/bench/metaTs/metaTs_spiced_RVMT.fastq.gz # Big 
fastq_file=/REDACTED_HPC_PATH/rolypoly/bench/sampled_0.05_metaTs_spiced_RVMT.fastq.gz # Small - Sampled 0.05% (proportaion) of the reads.


file_name=$(basename $(echo $fastq_file) .fastq.gz)

bash /REDACTED_HPC_PATH/rolypoly/rolypoly/filter_reads.sh -t $THREADS -M $MEMORY -i $fastq_file -D $known_DNA_present -r true -o "test_"$file_name -g "$file_name"_log.txt -S /REDACTED_HPC_PATH/rolypoly/
...
# fastq_file=/REDACTED_HPC_PATH/rolypoly/bench/metaTs/metaTs_spiced_RVMT.fastq.gz # Big 
fastq_file=/REDACTED_HPC_PATH/rolypoly/bench/metaTs/sampled_005_bb_metaTs_spiced_RVMT.fastq.gz # Small - Sampled 0.05% (proportaion) of the reads.

file_name=$(basename $(echo $fastq_file) .fastq.gz)

bash /REDACTED_HPC_PATH/rolypoly/rolypoly/in_reads/filter_reads.sh -t $THREADS -M $MEMORY -i $fastq_file -D $known_DNA_present -r true -o "test_"$file_name -g "$file_name"_log.txt -S $rolypoly_dir
