...
class OutputTracker:
    ...
    def add_file(self, filename: str, command_name: str, command: str, is_merged: bool, file_type: Optional[str] = None) -> None:
    ...
    def get_file_type(filename: str) -> str:
        ...
        file_types = {
            ...
            '.fq.gz': 'fastq_gzipped', '.fastq.gz': 'fastq_gzipped',
            '.fa': 'fasta', '.fasta': 'fasta',
            '.fa.gz': 'fasta_gzipped', '.fasta.gz': 'fasta_gzipped',
            '.txt': 'text', '.txt.gz': 'text_gzipped'
        }
    ...
    def get_latest_output(self, file_type: Optional[str] = None) -> Optional[str]:
        ...
        Example:
            ...
            ```
        """
        filtered_df = self.df.filter(pl.col("file_type") == file_type) if file_type else self.df
        return filtered_df.tail(1)["absolute_path"][0] if filtered_df.height > 0 else None
    ...
    def get_latest_non_merged_file(self) -> Optional[str]:
        ...
        return non_merged_files.tail(1)["absolute_path"][0] if non_merged_files.height > 0 else None
    ...
    def get_latest_merged_file(self) -> Optional[str]:
        ...
        return merged_files.tail(1)["absolute_path"][0] if merged_files.height > 0 else None
