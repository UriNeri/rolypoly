...
global virome_fastqs, metatranscriptome_fastqs, host_fastas, contigs_fastas, protein_fastas, fungal_fastq, fungal_host, hiv_fastqs, hiv_host, metagenomic_fastq, metagenomic_host, metagenomic_contigs, metagenomic_protein
...
def get_input_files(mode, command=None, small=False):
    ...
    file_dict = {
        "filter-reads": {
            "isolate_fungal": (f"{base_path}/isolate_fungi/*.fq.gz", f"{base_path}/isolate_fungi/*.fasta"),
            "isolate_hiv": (f"{base_path}/isolate_human/hiv/*.fq.gz", f"{base_path}/isolate_human/*.fasta"),
            "isolate_lyssa": (f"{base_path}/isolate_human/lyssa/*.fq.gz", f"{base_path}/isolate_human/*.fasta"),    
            "isolate_plant": (f"{base_path}/isolate_plant/*.fq.gz", f"{base_path}/isolate_plant/*.fasta"),
            "virome": (f"{base_path}/reads/meta/sea/*.fq.gz", None),
            "metatranscriptome": (f"{base_path}/reads/meta/soil/*.fq.gz", None),
            "metagenomic": (f"{base_path}/reads/meta/human/*.fq.gz", f"{base_path}/hosts/human/*.fasta"),
        },
        "filter-contigs": {
            "isolate_fungal": (f"{base_path}/isolate_fungi/example_virus.fna", f"{base_path}/isolate_fungi/*.fasta"),
            "isolate_hiv": (f"{base_path}/isolate_human/hiv/hiv.fna", f"{base_path}/isolate_human/*.fasta"),
            "isolate_lyssa": (f"{base_path}/isolate_human/lyssa/lyssa.fna", f"{base_path}/isolate_human/*.fasta"),
            "isolate_plant": (f"{base_path}/isolate_plant/plant_virus.fna", f"{base_path}/isolate_plant/*.fasta"),
        ...
        "assembly": {
            "isolate_fungal": (f"{base_path}/isolate_fungi/rp_filtered_reads/*.fq.gz",None),
            "isolate_hiv": (f"{base_path}/isolate_human/hiv/rp_filtered_reads/*.fq.gz",None),
            "isolate_lyssa": (f"{base_path}/isolate_human/lyssa/rp_filtered_reads/*.fq.gz",None),
            "isolate_plant": (f"{base_path}/isolate_plant/rp_filtered_reads/*.fq.gz",None),
            "virome": (f"{base_path}/reads/meta/sea/rp_filtered_reads/*.fq.gz",None),
            "metatranscriptome": (f"{base_path}/reads/meta/soil/rp_filtered_reads/*.fq.gz",None)
        },
        "annotate": {
        ...
        "rdrp-hmmsearch": {
            ...
            "metatranscriptome": (f"{base_path}/reads/meta/soil/example_metatranscriptome_contigs.fna", None)
        }
    }
...
@click.option("-m", "--mode", type=click.Choice(["isolate_fungal", "isolate_hiv", "isolate_lyssa", "isolate_plant", "virome", "metatranscriptome", "metagenomic", "contigs", "protein"]), default="isolate_fungal", help="Type of data to test")
...
def test_rolypoly(command, mode, small, output_dir, threads, memory, other_args):
    ...
    print(f"Testing rolypoly with command: {command}, mode: {mode}, small: {small}, output_dir: {output_dir}, threads: {threads}, memory: {memory}, other_args: {other_args}")
    ...
    if output_dir:
        ...
        args["output"] = str(RP_TESTS_DIR) + f"/rolypoly_tests/{command}_out_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
