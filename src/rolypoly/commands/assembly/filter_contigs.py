...
class FilterContigsConfig(BaseConfig):
    def __init__(self, **kwargs):
        ...
        self.mmseqs_args = kwargs.get("mmseqs_args") or "--min-seq-id 0.5 --min-aln-len 80"
...
@click.option("-i", "--input", required=True, type=click.Path(exists=True), help="Input path to fasta file")
@click.option("-d",  "--known-dna", "--host", required=True, type=click.Path(exists=True), help="Path to the user-supplied host/contamination fasta")
@click.option("-o", "--output", default=os.getcwd()+"/filtered_contigs.fasta", help="Output file location. ")
@click.option("-m", "--mode", type=click.Choice(['nuc', 'aa', 'both']), default='both', help="Filtering mode: nucleotide, amino acid, or both (nuc / aa / both)")
...
@click.option("--mmseqs-args", default="--min-seq-id 0.5 --min-aln-len 80", help="Additional arguments for MMseqs2")
@click.option("--diamond-args", default="--id 50 --min-orf 50", help="Additional arguments for Diamond")
...
def filter_contigs(input, known_dna, output, mode, threads, memory, keep_tmp, log_file, filter1_nuc, filter2_nuc,
                   filter1_aa, filter2_aa, dont_mask, mmseqs_args, diamond_args, overwrite, log_level):
    """
    ...
    config = FilterContigsConfig(
        ...
        output=Path(output).absolute().resolve(),
        threads=threads,
        log_file=Path(log_file) if log_file else Path(output).parent / f"rolypoly_filter_contigs_log.txt",
        memory=memory,
        mode=mode,
        keep_tmp=keep_tmp,
        overwrite=overwrite,
    ...
    config.logger.info(f"Contig filtering completed. Final output saved to {config.output}")
...
def filter_contigs_nuc(config: FilterContigsConfig):
    import pyfastx
    import polars as pl
    from rolypoly.utils.various import check_dependencies, apply_filter , ensure_memory
    from rolypoly.utils.fax import mask_dna, ensure_faidx, guess_fasta_alpha
    ...
    if config.input.suffix.endswith((".faa", ".fasta", ".fas", ".fna")):
        ...
        subprocess.run(["mmseqs", "createdb", str(config.input), str(input_db), "--dbtype", "2", "-v", "1"], check=True)
    ...
    if config.host.suffix.endswith((".faa", ".fasta", ".fas", ".fna", ".fa")):
        ...
        if not config.dont_mask:
            ...
            mask_args={"threads": config.threads ,
        ...
        subprocess.run(["mmseqs", "createdb", str(host_fasta), str(host_db), "--dbtype", "2", "-v", "1"], check=True)

    # Perform MMseqs2 search
    # config.logger.info(f"Searching against {host_db}")
    mmseqs_search_cmd = [
        "mmseqs", "search", str(input_db), str(host_db), f"{resdb}/res", str(config.tmp_dir),
        "--threads", str(config.threads), "-a", "--search-type", "3", "-v", "1"
    ]
    config.logger.info(f"Running mmseqs2 search with command:  {' '.join(mmseqs_search_cmd)}") 
    ...
    subprocess.run([
        "mmseqs", "convertalis", str(input_db), str(host_db), f"{resdb}/res",
        str(result_file), "--format-mode", "4",
        "--format-output", "qheader,theader,qlen,tlen,qstart,qend,tstart,tend,alnlen,mismatch,qcov,tcov,bits,evalue,gapopen,pident,nident", "-v", "1"
    ], check=True)
    ...
    if y.shape[0] == 0:
        config.logger.warning(f"No nucleic hits found for {config.input} against {config.host}, copying input to output")
    ...
    config.logger.info(f"Kept {filtered_sequences} sequences ({percentage_filtered:.2f}% filtered).")
    config.logger.info(f"Nucleotide filtering completed. Output saved to {config.output}")
...
def filter_contigs_aa(config: FilterContigsConfig):
    ...
    if config.host.suffix.endswith((".faa", ".fasta", ".fas", ".fna", ".fa")):
        ...
        if host_alpha == "nucl":
            ...
            subprocess.run(f"sed 's|\t|__|g' -i {str(host_fasta)}", check=True, shell=True)
        elif host_alpha == "amino":
            host_fasta = config.host
        else:
            config.logger.error(f"Can't guess the alphabet (doesn't look like nucl or amino fasta) of \n {config.host}")
            return

        if not config.dont_mask:
            masked_fasta = config.tmp_dir / "masked_host.fasta"
            rna_virus_prots = Path(os.environ.get('ROLYPOLY_DATA')) / "prots_for_masking.faa"
            diamond_mask_cmd = [
                "diamond", "blastp", "--query", str(host_fasta), "--db", str(rna_virus_prots),
                "--tmpdir", str(config.tmp_dir), "--threads", str(config.threads), "--un", str(masked_fasta)
            ]
            diamond_mask_cmd.extend(config.diamond_args.split())
            diamond_mask_cmd.extend(["--header", "simple", "--out", f"{config.tmp_dir}/diamond_out_for_masking.tab", "--outfmt", "6", 
                                     "qseqid sseqid pident length mismatch gapopen qlen qstart qend sstart send slen evalue bitscore qcovhsp"])
            with open(config.log_file, 'a') as log_file:
                subprocess.run(" ".join(diamond_mask_cmd), check=True, shell=True, stdout=log_file, stderr=log_file)
            host_fasta = masked_fasta

    # Perform the Diamond search    
    config.logger.info(f"Searching against {host_fasta}")
    diamond_search_cmd = [
        "diamond", "blastx", "--query", str(config.input), "--db", str(host_fasta),
        "--out", str(res_tab), "--tmpdir", str(config.tmp_dir), "--threads", str(config.threads)
    ]
    diamond_search_cmd.extend(config.diamond_args.split())
    diamond_search_cmd.extend(["--header", "simple", "--outfmt", "6", 
                               "qtitle sseqid pident length mismatch gapopen qstart qend qlen sstart send slen evalue bitscore qstrand qframe qcovhsp"])
    with open(config.log_file, 'a') as log_file:
        subprocess.run(" ".join(diamond_search_cmd), check=True, shell=True, stdout=log_file, stderr=log_file)

    # Apply filters
    config.logger.info(f"reading results from {res_tab}")
    y = pl.read_csv(res_tab, separator="\t", has_header=True)
    if y.shape[0] == 0:
        config.logger.warning(f"No amino acid hits found for {config.input} against {config.host}, proceeding to copy paste the input as the output.")
        shutil.copy(config.input, config.output)
        return
    ...
    config.logger.info(f"Filtered {len(y_set)} sequences.")
    config.logger.info(f"Kept {filtered_sequences} sequences ({percentage_filtered:.2f}% filtered).")
    config.logger.info(f"Amino acid filtering completed. Output saved to {config.output}")
