...
def run_subcommand(
    command: click.Command, args: list[str], label: str, logger
) -> None:
    ...
    try:
        ...
        if exc.code not in (0, None):
            raise click.ClickException(
                f"{label} failed with exit code {exc.code}"
            )


def prepare_combined_inputs(
    rdrp_fasta: Path, cp_fasta: Path, combined_fasta: Path, metadata_path: Path
) -> pl.DataFrame:
...
def build_presence_and_cluster_summary(
    ...
    if cluster_sample_long.is_empty():
        ...
        all_samples = sorted(
            cluster_sample_long["sample_id"].unique().to_list()
        )
        presence_wide = (
            cluster_sample_long.pivot(
                on="sample_id", index="cluster_id", values="present"
            )
            .fill_null(0)
...
def build_candidate_pairs(
    outdir: Path, cluster_summary: pl.DataFrame, write_single_rdrp_strict: bool
) -> pl.DataFrame:
    ...
    cluster_reps = extend_clusters.filter(pl.col("is_representative")).select(
        pl.col("cluster_id"), pl.col("contig_id").alias("representative_contig")
    )
    ...
    if "correlation" not in base_pairs.columns:
        base_pairs = base_pairs.join(
            correlation_pairs.rename(
                {"contig_1": "cluster_a_id", "contig_2": "cluster_b_id"}
            ),
...
def binit(
    ...
    from rolypoly.commands.bining.correlate import (
        correlate as correlate_command,
    )
    ...
    if (
        reuse_extend
        and extend_fasta_path.exists()
        and extend_clusters_path.exists()
    ):
        logger.info(
            "Reusing extend outputs at %s and %s",
            extend_fasta_path,
            extend_clusters_path,
        )
    else:
        run_subcommand(
            extend_command,
    ...
    logger.info(
        "Workflow complete. Candidate pairs: %s", putative_candidates.height
    )
