...
def found_label_expr() -> pl.Expr:
    ...
    return (
        ...
        .otherwise(
            pl.concat_str([pl.lit("rev_on_"), end_token, pl.lit("_end")])
        )
        .alias("found_label")
    )
...
def create_signature_frame(
    ...
    def build_end(end_name: str) -> pl.DataFrame:
        ...
        genome_end = (
            end_name
            if is_plus
            else ("3prime" if end_name == "5prime" else "5prime")
        )
        start_expr = (
            pl.lit(1)
    ...
    if strand in {"minus", "both"}:
        ...
        frames.append(
            create_signature_frame(
                df, "sequence_rc", min_len, max_len, "-", False
            )
        )
    frames = [frame for frame in frames if not frame.is_empty()]
    if not frames:
...
def aggregate_min_range_groups(
    signatures: pl.DataFrame, min_len: int, max_len: int, distance: int
...
def extend_group_motifs(
    ...
    join_df = (
        assignment_df.join(available, on=["contig_id", "end_label"], how="left")
...
def annotate_found_in_from_all_signatures(
    ...
    if (
        group_df.is_empty()
        or assignment_df.is_empty()
        or all_signatures.is_empty()
    ):
        return group_df
...
def finalize_group_output(group_df: pl.DataFrame) -> pl.DataFrame:
        ...
        return pl.DataFrame(
            {
                ...
                "clip_contains_source_ids": pl.Series(
                    [], dtype=pl.List(pl.Int64)
                ),
                "clip_contained_by_source_ids": pl.Series(
                    [], dtype=pl.List(pl.Int64)
                ),
                "members": pl.Series([], dtype=pl.List(pl.Utf8)),
            }
    ...
    columns_to_drop = [
        col for col in ["window_min", "window_max"] if col in group_df.columns
    ]
...
def is_one_edge_clipped_containment(
    shorter: str, longer: str, max_clipped: int
) -> bool:
...
def is_both_edge_clipped_containment(
    shorter: str, longer: str, max_clipped: int
) -> bool:
...
def collapse_groups_by_clipping(
    ...
    if len(rows) < 2:
        group_df = group_df.with_columns(
            pl.col("group_id")
            .map_elements(lambda gid: [gid], return_dtype=pl.List(pl.Int64))
            .alias("source_group_ids"),
            pl.lit([], dtype=pl.List(pl.Int64)).alias(
                "clip_contains_source_ids"
            ),
            pl.lit([], dtype=pl.List(pl.Int64)).alias(
                "clip_contained_by_source_ids"
        ...
        found_in = sorted(
            {
                ...
                for row in component_rows
                for entry in (
                    row.get("ends_present", []) or row.get("found_in", []) or []
                )
            }
        )
        contains_ids = sorted(
            {
                ...
                for nested in contains_map.get(gid, set())
            }
        )
        contained_by_ids = sorted(
            {
                nested
                for gid in source_ids
                for nested in contained_by_map.get(gid, set())
            }
        )
    ...
    remapped_assignments = assignment_df.with_columns(
        pl.col("group_id")
        .map_elements(
            lambda gid: membership_map.get(gid, gid), return_dtype=pl.Int64
        )
        .alias("group_id")
    )
...
def cluster_distance_groups(
    ...
    if not results:
        return [], []
    results.sort(
        key=lambda row: (row["end_label"], -row["member_count"], row["motif"])
    )
...
def build_membership_table(
    ...
    annotated = signatures.join(
        assignments, on=["contig_id", "end_label"], how="left"
    )
...
def write_group_motifs_fasta(
    ...
    to_write = to_write.with_columns(
        ...
        .otherwise(
            pl.col("example_seed_stop") - pl.col("motif_length_calc") + 1
        )
        .alias("example_start"),
        pl.when(pl.col("example_end") == "5prime")
    ...
    frame_to_fastx(
        fasta_df, fasta_path, seq_col="sequence", header_col="header"
    )
...
def termini(
    ...
    if output.suffix == "":
        logger.debug(
            "Output path missing suffix; defaulting to '.tsv' and TSV format"
        )
    ...
    raw_signatures = build_signature_table(
        seq_df, min_len, max_len, strand_mode
    )
    ...
    if not group_df.is_empty():
        group_df = extend_group_motifs(
            group_df, assignment_df, unique_signatures
        )
        if max_clipped_collapse:
            ...
            logger.info(
                "Second-pass clipped collapse disabled (--no-max-clipped-collapse)"
            )
    ...
    write_group_motifs_fasta(
        group_df, assignment_df, unique_signatures, motifs_fasta, logger
    )
