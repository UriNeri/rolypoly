...
def parse_args():
    parser = argparse.ArgumentParser(description="MSA Visualization CLI",
...
def draw_msa(input_file, start=1, end=None, wrap_length=360, show_consensus=False, markers=None, marker_x=None, text_annotations=None, output="output.pdf", dpi=360):
    mv = MsaViz(input_file, start=start, end=end, wrap_length=wrap_length, show_consensus=show_consensus)
    ...
    if text_annotations:
        for annotation in text_annotations:
            ...
            mv.add_text_annotation((int(start), int(end)), text, text_color="red", range_color="red")
